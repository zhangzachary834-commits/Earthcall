...
# ☀️ SUN CONTINUATION UPDATE — 2026-09-23

## Current live branch / PR

- Fresh branch: `sol/universal-singular-persistence-current-20260923`
- Base was current canonical `sync-from-earthcall-main` at `69860005066cc575bf6dc6efd4fde5ec5fbcf9b6`.
- Draft PR: **#342 — Universal Singular persistence on current default**
- This continuation deliberately did **NOT** merge or replay the old implementation branch wholesale. At inspection, `sol/singular-semantic-persistence-20260919` was 553 commits behind current default.

## What has been reconciled onto current default

The universal semantic-persistence substrate is now transplanted semantically:

- `Property::isSemanticallyWritable()` default false.
- `PropertyRef` and setter-backed `ComputedProperty` report writable semantics.
- `PropertyValueJson` now preserves typed Singular/Object/Relation/Formation references by stable identity and supports resolver-aware hydration.
- Nested list/dict values defer if an identity inside them is unresolved rather than partially mutating.
- OntoMath ScalarField/VectorField PropertyValues serialize their real payloads instead of type-only tombstones.
- `Singular` carries preserve-first deferred authored/registered JSON through copy/move.
- New universal `SingularPropertySerialization` envelope writes authored state, writable registered fallback state, designated zones, and preserves unresolved raw payloads until a resolver can bind them.
- Canonical root fields outrank the fallback envelope; unresolved raw state outranks placeholder/default live values on re-save.

Concrete roots reconciled so far:

- Object + custom ownership filter.
- Material.
- Relation + `attachment.*` ownership filter.
- Formation.
- Zone.
- Lexeme, including lexeme-only Zones (no Relation records required).
- FieldNode.
- BodyPart + its hidden primary Object.
- Body.
- Person + Soul + Joys + called Lexeme.
- FirstMover.
- Law + post-register late-reference rebinding.
- ObjectConcept + post-registry late-reference rebinding.
- Ourverse.

## Critical renderer reconciliation

Do **not** transplant the old FieldNode files.

Current canonical FieldNode has newer volumetric / radiance channels that did not exist on the old persistence branch:
...
The fresh implementation preserves those current canonical payloads and applies the universal envelope only around them. Registered paths under `field.*`, `vectorField.*`, `volume.*`, and `light.*` are excluded from fallback authority so there is only one truth.

## Object ownership reconciliation

Object canonical/mapped state is excluded from `registeredProperties` fallback when it already lives in the Object/Material/Matter codecs, including:

- transform / position / rotation / center
- authoritativeAxis / targetRotation / rotationResponsiveness
- material
- 2D position/order, renderMode, textString
...
Telos, physical state, rigid-form dynamics, authored Properties, and future writable registered paths with no canonical codec home remain eligible for the envelope.

## Regression witnesses

Fresh branch now contains:

- `tests/singularity/singular_property_persistence_matrix_test.cpp`
- `tests/zones/zone_singular_property_roundtrip_test.cpp`

The focused CI workflow has been updated to build both.

The matrix witness covers all PropertyValue alternatives, identity rebinding, OntoMath field payloads, stale-fallback precedence, and representative real persistence roots.

The Zone witness performs the real lifecycle:
**write Zone → destroy writer graph → create fresh ZoneManager → hydrate from identity store → verify identity-valued Properties point at the new beings.**

## Live CI state at this update

Latest PR-head workflow: **Earthcall focused CI run #2777** (`35844914843`).

At the moment this note was written:
- Focused CPU tests: in progress, checkout phase.
- SDF range-proxy verification: in progress.
- Slow Adapter independent clock: queued.

No failure from the new code has been observed yet; compilation of the new witnesses has not completed as of this timestamp.

## Next successor actions

1. Read PR #342 and the newest CI run before editing.
2. If Focused CPU build fails, fix the exact current-tree compile/test failure; do not retreat to the stale branch.
3. Run/inspect both universal persistence witnesses.
4. Re-audit any Singular persistence root added to canonical after this handoff update.
5. Only mark PR ready once the focused witnesses are green and no canonical codec is shadowed by duplicate fallback state.
