...
void PersonPerspective::handleInput(GLFWwindow* window) {
...
void PersonPerspective::activate() {
    _isActive = true;
}

void PersonPerspective::deactivate() {
    _isActive = false;
}

void PersonPerspective::setPosition(const glm::vec3& pos) {
    _viewState.position = pos;
    updateTargetFromPosition();
}

void PersonPerspective::setTarget(const glm::vec3& target) {
    _viewState.target = target;
    updatePositionFromTarget();
}

void PersonPerspective::setDistance(float distance) {
...
void PersonPerspective::setSensitivity(float sensitivity) {
    _settings.sensitivity = sensitivity;
}

void PersonPerspective::setMoveSpeed(float speed) {
    _settings.moveSpeed = speed;
}

void PersonPerspective::rotate(float deltaYaw, float deltaPitch) {
...
void PersonPerspective::zoom(float delta) {
    if (_viewState.isLocked) {
        return;
    }
...
void PersonPerspective::pan(const glm::vec3& delta) {
    if (_viewState.isLocked) {
        return;
    }
...
void PersonPerspective::updateTargetFromPosition() {
    if (_type == PerspectiveType::ThirdPerson) {
...
void PersonPerspective::updatePositionFromTarget() {
    if (_type == PerspectiveType::ThirdPerson) {
...
void PersonPerspective::updateTargetFromAngles() {
