...
namespace Core {
    void Engine::render() {
        ...
        // Rung 7 discovers radiant sources from the Zone's direct FieldNode
        // ownership index. It never asks one source AST to enumerate the world.
        ...
        if (radiantSources.size() == 1) {
            // Exact Rungs 3-6 compatibility: one source stays on the historical
            // scalar renderer path and therefore keeps its proven WGSL byte shape.
            ...
            // Mesh/fixed compatibility state still has one source-shaped slot.
            // Project the first enabled source there while WebGPU SDF transport
            // receives and sums the complete authored source collection.
