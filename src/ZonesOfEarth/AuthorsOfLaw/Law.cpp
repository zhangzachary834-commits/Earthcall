...
LawManager::~LawManager() {
    // Close global EventBus roads while every member they can touch is still
    // alive. EventBus also invalidates already-copied async snapshots before
    // unsubscribe returns, so no delivery can enter this manager afterwards.
    if (_ecaEventSubscription != 0) {
        Core::EventBus::instance().unsubscribe(_ecaEventSubscription);
        _ecaEventSubscription = 0;
    }
    if (_customEventSubscription != 0) {
        Core::EventBus::instance().unsubscribe(_customEventSubscription);
