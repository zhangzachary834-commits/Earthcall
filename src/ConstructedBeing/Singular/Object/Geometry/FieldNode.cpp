...
bool fieldNodeRegisteredPropertyNeedsEnvelope(const std::string& name) {
    if (name == "origin" || name == "scale") return false;
    if (name.rfind("field.", 0) == 0) return false;
    if (name.rfind("vectorField.", 0) == 0) return false;
    if (name.rfind("volume.", 0) == 0) return false;
...
nlohmann::json FieldNode::toJson() const {
    ...
    // Canonical mathematical payloads above remain the only authority for
    // FieldNode's mapped PropertyPaths. The universal envelope carries authored
    // state and only registered state with no stronger canonical home.
