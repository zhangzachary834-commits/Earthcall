...
- **Ontology First:** The top-level directory structure reflects the ontology of the substrate categories (`ConstructedBeing`, `Identity`, `Person`, `Relation`, `Singularity`, `Time`, `ZonesOfEarth`). It does not organize by technical subsystems. `Object`, `Lexeme`, and `Property` nest under `ConstructedBeing/Singular/`; Formation lives under `Relation/`. Ourverse the being lives under `ZonesOfEarth/`; the Person-facing tools live under `Singularity/FirstMoverWindowTools/`.
...
- Start with the manifesto in `docs/core/Earthcall Ourverse Manifesto/EarthcallOurverse.md`.
