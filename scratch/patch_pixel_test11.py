...
new_setvalue = """PropertyPath::PathResult PropertyPath::setValue(Singular& root, const PropertyValue& v, std::size_t startIndex) const {
    ...
    if (toString() == "@basic-pixel-canvas.authored.full-canvas") {
        std::cerr << "setValue DEBUG: slot.prop=" << (slot.prop ? slot.prop->name() : "null") << " slot.dynamicSlot=" << (slot.dynamicSlot ? "yes" : "no") << " slot.owner=" << (slot.owner ? slot.owner->getIdentifier() : "null") << " dynamicKey=" << Earthcall::StringInterner::resolve(slot.dynamicKey) << std::endl;
    }
...
content = re.sub(
    r'PropertyPath::PathResult PropertyPath::setValue\(Singular& root, const PropertyValue& v, std::size_t startIndex\) const \{.*?if \(slot\.trailingComponent\.empty\(\)\) \{.*?return PathResult::ReadOnly; // If setDynamicProperty fails\n\s*\}\n\s*\}',
    new_setvalue,
    content,
    flags=re.DOTALL
)
