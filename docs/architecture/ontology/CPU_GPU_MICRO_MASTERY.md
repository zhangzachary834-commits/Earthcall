...
An audit revealed a severe flaw in the original allocator: if a sub-allocation exceeded the remaining capacity of the current chunk, the pool would immediately allocate an entirely new chunk and skip the remaining capacity in all previous chunks for the duration of the frame. Under heavy load, this caused a cascading VRAM leak. The fix replaced the blind chunk jump with a linear search (`suballocateFrom`), ensuring existing memory is saturated before requesting new driver allocations.
