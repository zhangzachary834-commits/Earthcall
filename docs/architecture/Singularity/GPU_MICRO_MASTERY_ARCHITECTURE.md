...
**Correction (2026-08-26, verified against the landed implementation):** the last clause overstates what `RenderMode::Mesh` delivers today. The instanced batch collapse (§6) groups draws whose `MeshBatchKey::mesh` pointer is IDENTICAL, not merely equal in content — and `Object::rebuildGeometryCaches()` writes `_smoothMesh` as a per-Object member (`_smoothMesh = geom::tessellateSmooth(smoothData)`), so 1,000 separately-authored Toruses each own their own `TessMesh` at a distinct address. Setting `RenderMode::Mesh` on all of them produces 1,000 batches of size one — the tessellation cost is paid, Z-culling is real, but the draw-call count does not collapse. It genuinely batches only where Objects already share one static mesh instance (the merged-cube case in §4.2 of the remediation plan, `CPU_GPU_MICRO_MASTERY.md` §6-7 has the fuller account). Getting real instancing for a population of identical analytic shapes needs a content-addressed tessellation cache keyed on shape parameters rather than the Object — not built here.
