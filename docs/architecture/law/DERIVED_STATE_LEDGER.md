...
| `Prophetic::Index::_relevanceEdges`, `_unknownWriteSources`, `_relevanceComplete` | branch-local read demands + modeled write effects, plus explicit opaque-writer frontier, derived from the whole Law register | rebuilt with `_prophetic` on `Law::textRevision()`; opacity sets **incomplete** and removes narrowing authority but does not erase known edges; runtime must fall back whenever `_relevanceComplete == false` | `prophetic_rete_test` §H — JSON-stable branch provenance, `Any` arms, disjoint pair omission, known-edge retention under opacity, modeled First-Mover + unknown-source coexistence |
