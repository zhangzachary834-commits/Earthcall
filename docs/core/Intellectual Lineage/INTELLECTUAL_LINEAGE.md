...
The argument for it, stated in the field's vocabulary: you cannot enforce a policy against an adversary sitting below you in the trust stack. The set of things you are forced to trust is the Trusted Computing Base. Earthcall's currently includes macOS, the C++ runtime, GLFW, OpenSSL, the GPU driver, and the window manager — none of which has heard of a Person. Every Person guard in the tree is therefore advisory, not enforced. The manifesto knows this; ../../architecture/migration/SECURITY_FEATURES.md (397 lines) does not yet state a threat model or a trust boundary.
