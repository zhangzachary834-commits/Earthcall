...
5b. **Unify Geometry Framework with OntoMath (6-Rung Plan)** — PARTIAL (2026-08-17). Rungs 0 (Bernstein/monomial conversions, `binom`) and 2 (Bézier patches as bivariate `ScalarForm`s with exact symbolic normals) are **done and verified** — `tests/ontomath_test.cpp`, `tests/bezier_patch_law_test.cpp` (30 checks), `tests/geometry_ontomath_test.cpp` (37 checks). Rungs 1, 3, 4, 5 are **NOT done** and were marked done in error; see [Specific Tasks/Geometry_OntoMath_Remaining_Rungs.md](Specific%20Tasks/Geometry_OntoMath_Remaining_Rungs.md). (Ref: `docs/architecture/GEOMETRY_ONTOMATH_UNIFICATION_PLAN.md`.)
