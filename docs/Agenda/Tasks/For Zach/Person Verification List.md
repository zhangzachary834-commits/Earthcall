...
- [ ] **Perlin same-resolution contention control (Zach + Codex session `01a072e2`, corrected 2026-09-06 17:53 PDT):** Zach confirms the earlier 10–20 ms warm result already occurred around 2500x1574, so framebuffer size does not explain the regression. Launch only `/Users/zacharyzhang/Documents/GitHub/Earthcall/Run Earthcall.command`, load the Perlin Zone, choose one horizon camera, and do not move it. Record F3 `Framebuffer`, `Player Pos`, 3D phase, and surface-acquire wait after 120 frames. Repeat at the identical camera and framebuffer after fully hiding/minimizing Codex, Gemini, Chrome, and Antigravity (or quitting those Zach chooses), leaving only Earthcall visibly rendering. The live pre-test system showed 61–63% Apple GPU utilization with Earthcall closed and severe unified-memory pressure. Record whether isolation restores 10–20 ms; if it does not, runtime pipeline/cache/resource and generated-WGSL-hash instrumentation is required before another renderer change.
