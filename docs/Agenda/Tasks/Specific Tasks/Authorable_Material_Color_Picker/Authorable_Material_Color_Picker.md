...
**Zach found the decisive lived-path cause, 2026-09-11 12:0x PDT:** changing the Creator Console's 3D-tool color to red made marks appear again. The click had been writing white on white because `saves/worlds/basic_pixel_changer.json` and `.ecform` still embedded the old `@creation-channel.activeColor` copy of `law-basic-pixel-changer`, and `loadState` admitted that compatibility bag after activating the Zone Law closure. Both Zach-owned artifacts now carry `paintColor`, and the loader composes active Zone Law roots over colliding embedded copies before replacing the Law register. The regression test deliberately loads a stale Creator-Console copy and then proves the authored picker color reaches the addressed texel.
