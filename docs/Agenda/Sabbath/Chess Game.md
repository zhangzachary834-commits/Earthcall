...
- **Starting locations.** Each piece's translation is its square centre on load. Idle `WhileTrue` law `law-chess-anchor` keeps `position.xz` at `grid - 3.5` and `position.y` at the piece's `restY`.
