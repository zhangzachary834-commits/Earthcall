...
## Addendum — Unifying the Brush with Authored Math and Fields

**Originating connection by:** Jules / Claude (default harness)
**Session ID:** 7602167438967080663
**Date:** 2026-09-20

The Unified Brush System provides robust, traditional-feeling tools (opacity, flow, layers) for 2D and 3D painting. However, to fully align with Earthcall's trajectory, this system must interrelate with two emerging architectural capabilities:

1. **Authored Write Domains:** As defined in [`../Agenda/Tasks/Specific Tasks/Rendering and OntoMath/Authorable_Pixel_Writer/Authorable_Pixel_Writer.md`](../Agenda/Tasks/Specific%20Tasks/Rendering%20and%20OntoMath/Authorable_Pixel_Writer/Authorable_Pixel_Writer.md), the next rung of pixel writing is the ability to author the *mathematical set* of coordinates where writing is valid (e.g., using an OntoMath field). The brush system should not just paint blindly; its strokes should be bounded and modulated by these authored mathematical regions.

2. **Continuous Material Fields:** In [`../Agenda/Tasks/Specific Tasks/Rendering and OntoMath/OntoMath_Driven_Material_Fields/OntoMath_Driven_Material_Fields.md`](../Agenda/Tasks/Specific%20Tasks/Rendering%20and%20OntoMath/OntoMath_Driven_Material_Fields/OntoMath_Driven_Material_Fields.md), color is moving from discrete texels to continuous `OntoMath::Piecewise` expressions (`sdfColor(p)`). The brush system must bridge this gap: painting with a brush should either modify the parameters of these continuous color expressions, or the brush itself must be treated as a localized field operation rather than a destructive pixel override.

By tying the Brush System into the OntoMath fields and authored write domains, we move from a simple pixel-pusher to a true instrument of continuous, lawful creation.
