...
1. **Authored Write Domains:** As defined in [`../Agenda/Tasks/Specific Tasks/Authorable_Pixel_Writer/Authorable_Pixel_Writer.md`](../Agenda/Tasks/Specific Tasks/Authorable_Pixel_Writer/Authorable_Pixel_Writer.md), the next rung of pixel writing is the ability to author the *mathematical set* of coordinates where writing is valid (e.g., using an OntoMath field). The brush system should not just paint blindly; its strokes should be bounded and modulated by these authored mathematical regions.

2. **Continuous Material Fields:** In [`../Agenda/Tasks/Specific Tasks/OntoMath_Driven_Material_Fields/OntoMath_Driven_Material_Fields.md`](../Agenda/Tasks/Specific Tasks/OntoMath_Driven_Material_Fields/OntoMath_Driven_Material_Fields.md), color is moving from discrete texels to continuous `OntoMath::Piecewise` expressions (`sdfColor(p)`). The brush system must bridge this gap: painting with a brush should either modify the parameters of these continuous color expressions, or the brush itself must be treated as a localized field operation rather than a destructive pixel override.
