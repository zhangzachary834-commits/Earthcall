...
The important addition is the slow adapter's separation of preparation from use. It incrementally prepares routes for a Law's `Related(kind, category)` conjuncts. The evaluation path can use a prepared candidate set when its currency is established, and otherwise use the existing sweep. See [the implementation account](../Agenda/Tasks/Specific%20Tasks/Formation_Rete/Formation_Rete.md) and [SlowAdapter.cpp](../../src/Relation/Traversal/SlowAdapter.cpp).
