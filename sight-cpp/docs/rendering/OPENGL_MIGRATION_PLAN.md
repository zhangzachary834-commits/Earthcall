...
Two things were wrong here independent of which API comes next. **Both are now
fixed** — defect 1 by Milestone 1 (geometry caching), defect 2 by deleting the stub
(commit retiring `GL3Renderer`). The write-ups are kept as the record of why those
milestones existed.

**1. Geometry is re-tessellated every frame. — FIXED (Milestone 1).**
...
(Milestone 2). Both files and every `USE_GL3_RENDERER` block are now removed; the
`Engine.cpp` ImGui-backend selection collapsed to the live OpenGL2 path (that seam is
where `imgui_impl_wgpu` plugs in at Milestone 5).
...
- **The view comes from Perspective.** `PersonPerspective::getProjectionMatrix` already
...
The Perspective *concept* does not change under WebGPU. `PerspectiveType` (first-person,
third-person, top-down, isometric, free), `ViewState`, `CameraSettings`, the input
handlers, and `BasicPerspective/Perspective : public Singular` are all glm and GLFW —
neither of which the migration touches. What changes is the *plumbing between Perspective
and the GPU*, and the change makes Perspective more central, not less.

### There are currently two cameras, and the wrong one is authoritative

`PersonPerspective` already speaks the modern language. Its matrix methods are pure glm,
with zero GL:
...
glm::mat4 PersonPerspective::getProjectionMatrix(float aspectRatio) const {
    return glm::perspective(glm::radians(_settings.fov), aspectRatio,
                            _settings.nearPlane, _settings.farPlane);
}
```

But these methods have **no callers outside the module.** The camera that actually drives
rendering is a *separate* system — the `_camera` struct in `GameRender.cpp`, built on the
fixed-function matrix stack:

```cpp
glFrustum(left, right, bottom, top, nearZ, farZ);   // projection onto the GL stack
ecgl::lookAtMul(eyePos, lookTarget, _camera.up);     // view (was gluLookAt)
glGetDoublev(GL_MODELVIEW_MATRIX,  _camera.modelview);   // read it back out of the driver
glGetDoublev(GL_PROJECTION_MATRIX, _camera.projection);
```

So today the fixed-function pipeline owns the *real* camera, and the class literally named
after the manifesto's Perspective concept sits inert, producing matrices nobody reads.

### WebGPU has no matrix stack, which forces unification

There is no `glFrustum`, no `glMultMatrix`, no `glGetDoublev` in WebGPU. View and
projection must be computed on the CPU and handed to the shader as a uniform buffer:
...
`PersonPerspective` already produces exactly those two matrices. So the migration does not
*change* Perspective — it **deletes the fixed-function `_camera` and promotes
`PersonPerspective` to the single source of truth.** The plumbing swap:

| Today (fixed-function) | Under WebGPU |
|---|---|
| `glFrustum(...)` builds projection on the GL stack | `PersonPerspective::getProjectionMatrix()` → uniform |
| `ecgl::lookAtMul(...)` builds view on the GL stack | `PersonPerspective::getViewMatrix()` → uniform |
| `glGetDoublev(GL_MODELVIEW_MATRIX, ...)` reads it back | already held — no readback |
| `_camera.modelview/projection` (`GLdouble[16]`) | `glm::mat4` straight into the uniform buffer |

The third row is the tell: the `glGetDoublev` round-trip exists *only* because the driver
computed the matrix and we had to ask for it back. Under WebGPU we compute it, so we already
hold it. That round-trip — and the `ecgl::project`/`unProject` picking shim, which consumes
those `GLdouble[16]` arrays — both simplify: picking takes `glm::mat4` from
`PersonPerspective` directly instead of the driver's read-back doubles.

### Why this is the manifesto's architecture, not an accident

The manifesto: *"The Perspective of the substrate itself combined with Person Perspective."*
That is two perspectives — the substrate's framing (projection, viewport, FOV) and the
Person's standpoint (the view matrix). Today both live in the fixed-function `_camera` while
`PersonPerspective` is inert; that is backwards from what the manifesto describes. Because
WebGPU refuses to own a camera, Earthcall's Perspective ontology has to — the projection
becomes the substrate's framing, the view becomes the Person's standpoint, and
`Perspective : public Singular` becomes the real anchor rather than a stub.

### A free step available now

Independent of any backend: wire `GameRender` to *use*
`PersonPerspective::getViewMatrix()` / `getProjectionMatrix()`, feeding the results into the
existing `glMultMatrixf`, and delete the duplicate `_camera` orientation logic. This unifies
the two camera systems while still on OpenGL — same payoff-before-commitment shape as caching
the tessellations (Milestone 1). When Milestone 5 arrives, the camera is then already solved:
the uniform buffer is fed by a source of truth that already exists. See Milestone 5.
...
### Milestone 1: Stop Re-Tessellating — DONE
...
### Milestone 2: Renderer Boundary — DONE
...
- migrated draw paths — DONE for **every object surface path**: the topology/TessMesh paths
  ...
  meshes built once (statics); the polyhedron is a per-object face-mesh cache (`_polyhedronFaceMeshes`,
  rebuilt when `polyhedronData` changes via `_polyhedronDirty`). Each resolves its Object's material
  by identifier. The selection *glow overlay* (`drawHighlightOutline`) also went behind the interface
  via two new verbs — `drawLines` (wireframe) and `drawOverlay` (additive glow shell) — and open-surface
  two-sidedness became `RenderMaterial::doubleSided` (GL two-sided model; WebGPU cull-none). **`ObjectRender.cpp`
  now contains ZERO raw GL** — every object draw path is behind `Renderer`.
- ~~**deletion** of `GL3Renderer` and `USE_GL3_RENDERER`~~ — DONE.

Renderer contract as it stands: `drawMesh`, `drawImplicit`, `drawLines`, `drawOverlay`. WebGpuRenderer (M5)
implements exactly these.

Tests: `primitive_render_test` covers cube/sphere/cylinder/cone/polyhedron + the overlay verbs reaching the
framebuffer through the renderer + material tinting; `geometry_cache_test` / `material_render_test` cover the
topology paths and default pixel-parity.

Behaviour: the default material reproduces the old global shading exactly — the render tests
(`geometry_cache_test`, `material_render_test`) confirm default renders are pixel-identical and a
tinted material reaches the framebuffer.

Fixed-function honesty: `OpenGLRenderer` realizes `baseColor` + `shininess` per object; `specular`
and the `ambient`/`diffuse` coefficients ride on `RenderMaterial` for the WGSL shader at M5, where
per-material lighting is actually evaluated (fixed-function GL never set a material specular, so
there are no highlights today and adding them now would break default parity).
...
Design note (RESOLVED): `Material` lives as its own being, referenced by Objects via identifier
(the `Relation`-endpoint model). See "Person-Authored Math" / the Material sections above.

### Milestone 3: Retire GLU — DONE
...
- `WebGpuRenderer` against the Milestone 2 interface — implements the 4 verbs
  (`drawMesh`, `drawImplicit`, `drawLines`, `drawOverlay`).
- GLFW in `GLFW_NO_API` mode + `CAMetalLayer` surface
- ~~`wgpu-native` vendored~~ — DONE & PROVEN. `third_party/wgpu/` holds v29.0.1.1
  (macos-aarch64): `include/webgpu/{webgpu,wgpu}.h`, `lib/libwgpu_native.{a,dylib}`.
  A smoke test (`wgpuGetVersion` + `wgpuCreateInstance`) compiles, **static-links,
  and runs** in this environment — so WebGPU is feasible. WORKING LINK RECIPE
  (static, self-contained — no rpath needed):
  ```
  -I third_party/wgpu/include
  third_party/wgpu/lib/libwgpu_native.a
  ...
  ```
  Note: `-lwgpu_native` against the `.dylib` links but fails at runtime (no
  LC_RPATH); use the `.a` by path, or add `-Wl,-rpath,@loader_path/...`. The API
...
- **Offscreen render PROVEN** — `src/Rendering/WebGPU/smoke_offscreen.cpp` (run via
  ...
  whole substance of `drawMesh` minus a pipeline — work headless on this machine.
  v29 API notes for the renderer build: async requests use `*CallbackInfo{ mode,
  callback, userdata1, userdata2 }` with `WGPUCallbackMode_AllowProcessEvents`,
  pumped by `wgpuInstanceProcessEvents` / `wgpuDevicePoll(dev,true,nullptr)`;
  labels are `WGPUStringView{ptr,len}`; texture→buffer copy is `WGPUTexelCopy*Info`
  (renamed from ImageCopy); `bytesPerRow` must be 256-aligned; color-attachment
...
- **`WebGpuRenderer` class PROVEN** — `src/Rendering/WebGPU/WebGpuRenderer.{hpp,cpp}`
  ...
  `setCamera → beginFrame → setModel → drawMesh(TessMesh, RenderMaterial) → endFrame` —
  rendering a lit red quad (normal +Z) offscreen and reading back `(122,0,0,255)` = exactly
  `(0.2 + 0.8·N·L)·255`. So the real `TessVertex` layout (pos+normal+uv, stride 32), the
  model/camera/material uniforms, the normal transform, and two-sided Lambert lighting all
  work through the real class. Per-draw vertex+uniform buffers are created and released each
  frame (caching is a later optimisation).
- **Depth + perspective + texture albedo PROVEN** (extended `make webgpu-renderer`):
  - Depth24Plus depth buffer (recreated on target-size change) + `depthCompare Less` in the
    pipeline. Scene 1 draws a near BLUE quad BEFORE a far RED quad under a real
    `glm::perspective` camera and reads back `(0,0,122,255)` — the near quad wins, so depth
    testing works. The test TU sets `GLM_FORCE_DEPTH_ZERO_TO_ONE` (WebGPU clip depth is [0,1]);
    the app's projections must do the same when feeding this backend.
  - Texture albedo: `RenderMaterial` gained portable `albedoPixels`/`albedoSize` (the same RGBA8
    the FaceTexture keeps on the CPU; the GL `textureId` isn't portable). WebGpuRenderer uploads
    them to a WGPU texture, samples in-shader (`baseColor * texel * lit`), with a 1×1 white
    fallback + a shared linear sampler so untextured materials still satisfy the 3-entry bind
    group. Scene 2 (white material + green albedo) reads back `(0,122,0,255)`.
- **Rendering polish DONE** — the WebGpuRenderer mesh path is now feature-complete, all verified
  by the 5 scenes in `make webgpu-renderer`:
  - **Blinn-Phong specular**: shader gained an `eyePos` uniform (via `setCamera(viewProj, eyePos)`)
    and a `pow(N·H, shininess)` white highlight gated to lit faces. Scene 3 (red quad, normals along
    the half-vector) reads `(255,173,173)` — green/blue jump from 0, which only specular can do.
  - **`drawLines` + `drawOverlay`**: a shared unlit flat-colour pipeline (`kFlatWGSL`, position-only)
    in three variants — additive triangles / alpha triangles / line-list — all depth-tested but not
    depth-writing (overlays must not occlude each other). Scene 4 (additive green shell) reads
    `(0,128,0)`; scene 5 (yellow wireframe box) draws 12 px. NOTE: WebGPU line primitives are always
    1px, so the OpenGL multi-pass line *width/glow* doesn't translate — the wireframe still reads.
  - Each verb sets its own pipeline (mesh/overlay/lines) so a pass can interleave them per object.
  DEFERRED (not done, on purpose): vertex/uniform/texture **caching** — currently every draw creates
  and releases its buffers. It's a pure optimisation with *no measurable benefit until WebGPU is the
  live backend*, and the right cache design (keying, invalidation) depends on the real per-frame
  usage pattern, which we'll only know after integration. Premature to build now.
  INTEGRATION — **frame lifecycle on the interface: DONE**. `Renderer` gained
  `beginFrame(w,h,clearColor)` / `endFrame()` as virtuals with **empty defaults**, so OpenGL
  inherits no-ops (GL is immediate; `GameRender`'s existing `glClear` is untouched) and only the
  WebGPU backend overrides them. `GameRender::render()` now brackets its drawing:
  `currentRenderer().beginFrame(fbW, fbH, {zone.r,g,b,1})` right after `glClear`, and
  `currentRenderer().endFrame()` at the end. Under OpenGL these do nothing — verified: app builds,
  all render tests pass, output unchanged. WebGpuRenderer's offscreen entry was renamed
  ...
  the surface texture, renders a spinning lit cube through `WebGpuRenderer.beginFrameOffscreen →
  drawMesh → endFrame`, and `wgpuSurfacePresent`s. **Compiles and links clean; needs a real display
  to run — user verifies on their Mac.** Required a format param on `WebGpuRenderer::init`
  (BGRA8Unorm for the surface vs RGBA8Unorm offscreen) since the pipeline color target must match
  the attachment. This is the standalone proof; once confirmed, the same surface code moves into
  `Engine` to make WebGPU the app's live backend.
  STILL TODO after the demo checks out: move the surface into `Engine` (behind a flag) + point
  `setCurrentRenderer` at the WebGpuRenderer; fill the live `beginFrame` to acquire the surface;
  the imgui backend swap (`imgui_impl_wgpu`); unify the camera; and build the app's projections with
  `GLM_FORCE_DEPTH_ZERO_TO_ONE` for the WebGPU backend.
...
- camera as a uniform buffer fed by `PersonPerspective::getViewMatrix()` /
  `getProjectionMatrix()`; the fixed-function `_camera` (glFrustum + `ecgl::lookAtMul`
  + `glGetDoublev`) is deleted and picking reads `glm::mat4` directly. WebGPU has no
  matrix stack, so this unification is forced here — see "Perspective And The Camera".
  The camera can be unified *before* this milestone while still on OpenGL; doing so
  early removes it from the critical path here.
